...
`settlement-layer` is the most important directory as it contains the base logic to use a given settlement layer as well as the actual settlement layers that are installed into your project. It also contains modules, which are tied to a settlement layer.
