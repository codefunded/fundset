...
import { erc20Address } from './erc20-utils.ts';
import { RpcError, ValidationErrors } from './schema.ts';
...
export const isTokenSupported = async (token: Address) => {
  if (token !== erc20Address) {
    throw new RpcError('Token is not supported', ValidationErrors.InvalidFields);
  }
};
